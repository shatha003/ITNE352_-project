...
def handle_client(client_socket, client_address):
    try:
        ...
        while True:
            ...
            if not endpoint:
                client_socket.send(json.dumps({"status": "error", "message": "Invalid option"}).encode())
                continue

            errors = validate_params(params)
            if errors:
                client_socket.send(json.dumps({"status": "error", "message": errors}).encode())
                continue

            api_response = fetch_news_data(endpoint, params)
            filename = f"{client_name}_{option}.json"
